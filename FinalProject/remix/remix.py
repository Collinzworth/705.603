...
class Remix(Sequence):
    ...
    def find_transition_points(self):
        transition_beats = []
        remix_beats = self._beats
        for beat_num, beat in enumerate(remix_beats):
            if beat_num + 1 < len(remix_beats):
                next_beat = remix_beats[beat_num+1]
                if beat.track != next_beat.track:
                    transition_beats.append(beat_num)
        self.transition_beats = transition_beats
